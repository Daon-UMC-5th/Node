...
const writeBoard = async (param, body, user) => {
  try {
    ...
    if (returnData == -1) {
      ...
      return returnData;
      // return await oneBoardDTO(await returnWriteBoardData (returnData));
    }
  } catch (error) {
    throw error;
  }
...
// img
const imageUpload = async (board_id, img) => {
  try {
    returnData = await imgUp({
      board_id,
      img,
    });
    if (returnData == -1) {
...
const modifyBoard = async (param, body) => {
  try {
    ...
    if (returnModifyData == -1) {
      ...
      return await oneBoardDTO(await returnWriteBoardData(returnModifyData));
    }
  } catch (error) {
    throw error;
  }
...
const postLikeComment = async (param, user) => {
  try {
    ...
    if (likeUpCommentData == -1) {
      throw response(status.INTERNAL_SERVER_ERROR);
    } else {
      return await countLikeCommentData(likeUpCommentData);
    }
  } catch (error) {
    throw error;
  }
...
const deleteLikeComment = async (param, user) => {
  try {
    ...
    if (likeDownCommentData == -1) {
      throw response(status.INTERNAL_SERVER_ERROR);
    } else {
      return await countLikeCommentData(likeDownCommentData);
    }
  } catch (error) {
    throw error;
  }
